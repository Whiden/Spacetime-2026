...
9. [Schematics & Blueprints](#9-schematics--blueprints)
10. [Ships](#10-ships)
...
**Extraction** (Exploitation, Agricultural) — per infrastructure level:
```
output = 1 × richness_modifier
```

**Manufacturing** (Low/Heavy/High-Tech/Space Industry) — per infrastructure level:
```
output = 1 unit of product, consuming 1 unit of each required input
```
If inputs are in shortage, output is **halved** (not zero).

### Infrastructure Caps
- **Civilian**: Uncapped.
- **Extraction/Agricultural**: Capped by deposit richness (Poor=5, Moderate=10, Rich=15, Exceptional=20).
...
### Patents
Patents are general-purpose technological advantages owned by corporations of any type. They improve how the corporation operates — contract speed, extraction efficiency, investment returns. Patents are unlocked by discoveries.
...
The UI naturally surfaces: "No active contracts" → Create Contract button. "No corporations" → guidance about kickstarting via contracts. "No ships" → explains prerequisites. Dashboard shows Terra Nova with clear investment opportunities.
